...
- [ ] `lib/db/database.dart`（新規作成）を作成し、`@DriftDatabase(tables: [Categories, Feeds, Posts])` アノテーションをつけて `AppDatabase` クラスを定義する。
- [ ] データベース接続ロジック（`NativeDatabase` 等）を実装する。
- [ ] `AppDatabase` 内にDAO（Data Access Object）的なメソッド、または別ファイルでDAOを作成し、以下のクエリを実装する。
  - [ ] フィード一覧取得・保存・削除（FeedとCategoryのJOINを含む）
  - [ ] 投稿一覧取得・保存（PostとFeedのJOINを含む）
  - [ ] 特定のフィードIDに基づく投稿取得・ソート（`pubDate`降順）
  - [ ] タイトルまたは内容による投稿の検索機能
  - [ ] 未読/既読状態の更新（単一・一括）
  - [ ] カテゴリの保存・一覧取得・名前による検索
- [ ] `flutter pub run build_runner build --delete-conflicting-outputs` を実行し、Driftの生成コード（`database.g.dart`等）を生成する。
