...
find lib/helpers -type f -name "db_helper.dart" -print0 | xargs -0 sed "${SED_ARGS[@]}" -e "s/import 'package:meread\/db\/database.dart' hide Feed, Category, Post;/import 'package:meread\/db\/database.dart' as db;/g"
find lib/helpers -type f -name "db_helper.dart" -print0 | xargs -0 sed "${SED_ARGS[@]}" -e "s/AppDatabase/db.AppDatabase/g"
